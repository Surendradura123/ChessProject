...
// To be completed...
  ...
  Method to find all the White Pieces.
    ...
    /*
    Stack white attacting square moves
    */
    private Stack getWhiteAttackingSquares(Stack pieces) {
        ...
        while (!pieces.empty()) {
            ...
            if (tmpString.contains("knight")) {
                ...
                Stack tempK = getBishopMoves(s.getXC(), s.getYC(), s.getName());
                while (!tempK.empty()) {
                    Square tempBishop = (Square) tempK.pop();
                    pieces.push(tempBishop);

                }
              } else if (tmpString.contains("Rook")) {
                  Stack tempK = getRookMoves(s.getXC(), s.getYC(), s.getName());
                  while (!tempK.empty()) {
                      Square tempRook = (Square) tempK.pop();
                      pieces.push(tempRook);

                  }
                } else if (tmpString.contains("BlackPawn")) {
                    Stack tempK = getWhitePawnSquares(s.getXC(), s.getYC(), s.getName());
                    while (!tempK.empty()) {
                        Square tempPawn = (Square) tempK.pop();
                        pieces.push(tempPawn);

                    }
                  } else if (tmpString.contains("Queen")) {
                      Stack tempK = getQueenMoves(s.getXC(), s.getYC(), s.getName());
                      while (!tempK.empty()) {
                          Square tempQueen = (Square) tempK.pop();
                          pieces.push(tempQueen);

                      }
                    } else if (tmpString.contains("King")) {
                        Stack tempK = getKingSquares(s.getXC(), s.getYC(), s.getName());
                        while (!tempK.empty()) {
...
/************************************************************************************************************************************************************/
    ...
    public static void main(String[] args) {
      ChessProject c = new ChessProject();
      c.startGame();
    }
}
